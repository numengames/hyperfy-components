  ...
  if (world.isClient) {
    const baseConfig = [
      ...
      },
      {
        ...
        label: 'Node name',
        type: 'text',
        hint: 'Optional: The name of the 3D node to use as visual trigger (leave empty if not needed)'
      },
      {
        key: 'enableDebugMode',
        label: 'Debug Mode',
        type: 'toggle',
        initial: false,
        hint: 'Enable detailed logging to help troubleshoot redirect issues'
      },
      {
        key: 'enableCollision',
        type: 'toggle',
...
class RedirectController {
  ...
  static getConfig() {
    return [
      ...
      },
      {
        ...
        type: 'text',
        initial: '',
        hint: 'The URL to redirect to (e.g., https://example.com)'
      },
      {
        key: 'openInNewWindow',
        ...
        type: 'toggle',
        initial: true,
        hint: 'Open the URL in a new browser window/tab'
      },
      {
        key: 'redirectControllerAcceptAnyEmitter',
      ...
      {
        key: 'redirectControllerEventReceivers',
        label: 'Event Listeners',
        type: 'textarea',
        initial: '[]',
        hint: 'JSON array of event configurations. Example: [{"id":"eventName","actions":[{"type":"redirect","params":{}}]}]'
      },
    ];
  }

  static EVENTS = {
    REDIRECT_COMPLETE: 'redirect-complete',
    REDIRECT_SERVER_TO_CLIENT: 'redirect-sc',
    REDIRECT_COMPLETE_TO_SERVER: 'redirect-complete-cs',
  };
  ...
  constructor({ app, node, props, world }) {
    ...
    this.props = props;
    this.world = world;
    this.log = createDebugLogger(`${this.props.appID} - RedirectController`);

    this.log('info', 'RedirectController instance created');
  ...
  _getActionHandlers() {
    return {
      'redirect': (ctx, payload) => ctx._onRedirectEvent(payload)
    };
  }

  _onRedirectEvent(params = {}) {
    this._executeRedirect(params);
  }
  ...
  _initEventListeners() {
    ...
    let receivers = this._normalizeReceivers(this.props.redirectControllerEventReceivers);
    ...
    if (this.props.redirectControllerAcceptAnyEmitter) {
      this.world.on(this.props.appID, (data) => {
        if (this.world.isServer) {
          this.log('info', 'Server received redirect request, broadcasting to clients');
          this.app.send(RedirectController.EVENTS.REDIRECT_SERVER_TO_CLIENT, data);
        } else {
          this._handleEvent('redirect', {}, data);
        }
      });
    }

    if (this.world.isClient) {
      this.app.on(RedirectController.EVENTS.REDIRECT_SERVER_TO_CLIENT, (data) => {
        this.log('info', 'Client received redirect request from server');
        this._handleEvent('redirect', {}, data);
      });
    }

    if (this.world.isServer) {
      this.app.on(`${RedirectController.EVENTS.REDIRECT_COMPLETE_TO_SERVER}-${this.props.appID}`, (data) => {
        this.log('info', 'Server received redirect completion, broadcasting to clients');
        this.app.emit(`${RedirectController.EVENTS.REDIRECT_COMPLETE}-${this.props.appID}`, data);
      });
    }
...
if (typeof module === 'undefined') {
  try {
    ...
    initAppConfig(RedirectController.getConfig());

    const node = findNodeByName(props.targetNodeName);

    initializeNodeVisibility(node);

    new RedirectController({ app, node, props, world });
